...
> 💡 **Novo (Mar/2026):** Heartbeats agora têm auto-recovery — se um check falhar, o próximo ciclo roda normalmente sem travar o loop inteiro. Não precisa mais de fallback manual pra heartbeat travado.
> 
> 💡 **Novo:** O gateway agora expõe `/v1/models` e `/v1/embeddings` (compatível OpenAI API). Isso permite integrar com ferramentas externas que falam protocolo OpenAI.
